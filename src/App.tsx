...
function App() {
  ...
  function handleClickAvatar(e: React.MouseEvent<HTMLDivElement, MouseEvent>) {
    const node = e.target as HTMLElement
    const clientRect: DOMRect = node.getBoundingClientRect()
      ...
      <div className="flex items-center justify-center h-screen">
        <div className="overflow-hidden">
          <div className="relative">
            ...
            {isShowDropDown && (
              <DropdownContent coords={coords} position={"right"} />
            )}
          </div>
        </div>
      </div>
...
const DropdownContent = (props: { coords: Coords; position: Position }) => {
  const { coords, position } = props
  return createPortal(
    <div
      style={{
        top: coords.y + coords.height * 1.4,
        [position]: coords.x,
      }}
      className="dropdown-content absolute top-full bg-white rounded-2xl shadow z-10 w-[230px] py-6 px-5"
    >
